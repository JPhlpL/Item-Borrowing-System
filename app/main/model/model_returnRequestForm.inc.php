...
// Getting the List of Customer List in specific row vs inside Inventory
$get_child_idSql = "SELECT a.id id,
                    a.IT_REQUEST_CONTROL_NUMBER currentControlNumber,
                    a.IT_REQUEST_ITEM_NAME currentItemName,
                    a.IT_REQUEST_ITEM_REMARKS currentItemRemarks,
                    a.IT_REQUEST_ITEM_STATUS currentItemStatus,
                    b.IT_ITEM_QUANTITY inventoryQty,
                    a.IT_REQUEST_ITEM_QUANTITY currentItemQuantity,
                    (b.IT_ITEM_QUANTITY + a.IT_REQUEST_ITEM_QUANTITY) AS totalsum,
...
if(empty($get_child_idRow['itemName']))
{
    // Getting the List of Customer List in specific row vs inside Inventory
...
<section class="content">
          ...
          <div class="invoice p-3 mb-3">
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $get_child_idRow["currentItemStatus"];?></td> 
                                    <td>
                                        <?php 
                                    ...
                                    <td>
                                        <?php 
                                            if(isset($get_child_idRow["totalsum"]))
                                            {
                                                if($get_child_idRow["totalsum"] <= 0){
                                                    echo "<label class='text-danger'>".$get_child_idRow["totalsum"]."</label>";
                                                }
                                                else{
                                                    echo "<label class='text-success'>".$get_child_idRow["totalsum"]."</label>";
                                                }
                                            }
                                            else{
                                                echo $get_child_idRow["currentItemQuantity"];
                                    ...
                                    <td>
                                        <?php if($get_child_idRow["currentItemQuantity"] != 0) {?>
                                        <a href="javascript:void(0)" data-toggle="tooltip" title="Edit Data" id="btn-edit" class="btn btn-default" data-id="<?php echo $get_child_idRow['id']; ?>"> 
                                            <i class="fas fa-arrow-circle-down text-info"></i> Return
                                        </a> 
                                        <?php } else{  ?>
                                            <label class='bg-success border rounded'><i class='p-1 nav-icon fas fa-check'>Returned</i></label>
                                        <?php }?>
                ...
                <div class="row justify-content-between">
                    ...
                    <div class="col-2t">
                        ...
                        <button type="submit" id="returnBtn" name="Return" class="btn btn-default"><i class="fas fa-arrow-circle-down text-info"></i> Return </button>
                    </div>
                </div>
          </div>
          <!-- /.invoice -->
  ...
  <?php include '../crud_modal/tblReturn.inc.php'; ?>
